...
export function dashboard(username, modules) {
  ...
  return `${head('NEXUS404')}<body class="dashboard-page"><div class="page">${header(username)}<main><div class="section-line"><h1>Личное пространство</h1><span>${modules.length} подключено</span></div>${!modules.length ? '<p class="settings-empty">Пока нет модулей.</p>' : ''}<div class="modules-grid">${cards}</div>${settingsCard}</main>${footer}</div>${dialog}</body></html>`;
}
