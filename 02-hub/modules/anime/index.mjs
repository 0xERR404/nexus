...
const content = `${head}<section id="animePage">${status}<div class="anime-toolbar"><input id="animeSearch" type="search" maxlength="200" aria-label="Поиск аниме" placeholder="Найти аниме"><select id="animeFilter" aria-label="Статус просмотра"><option value="">Все статусы</option><option value="watching">Смотрю</option><option value="completed">Посмотрел</option><option value="planned">Запланировано</option><option value="on_hold">Отложено</option><option value="dropped">Бросил</option><option value="rewatching">Пересматриваю</option></select><select id="animeSort" aria-label="Сортировка аниме"><option value="name">По названию</option><option value="score">По моей оценке</option><option value="progress">По % просмотра</option><option value="watched">По числу серий</option></select><button id="animeSync" type="button">Обновить</button></div><div class="anime-meta"><span id="animeCount"></span><span id="animeTime"></span></div><div id="animeList" class="anime-list"></div><dialog id="animeDetailDialog" aria-labelledby="animeDetailTitle"><div class="anime-meta"><h2 id="animeDetailTitle"></h2><button id="animeDetailClose" class="dialog-close" aria-label="Закрыть">×</button></div><p id="animeDetailStatus" role="status"></p><div id="animeDetailBody"></div><button id="animeDetailRetry" hidden>Повторить</button></dialog></section>`;
    ...
    async handle({request, path: route, user}) {
      try {
        if (['GET', 'HEAD'].includes(request.method)) {
          ...
          const detail = /^\/detail\/([1-9]\d{0,9})$/.exec(route);
          if (detail) return Response.json(await store.detail(Number(detail[1])));
