...
const content = `${head}<section id="animePage">${status}<div class="anime-toolbar"><input id="animeSearch" type="search" maxlength="200" aria-label="Поиск аниме" placeholder="Найти аниме"><select id="animeFilter" aria-label="Статус просмотра"><option value="">Все статусы</option><option value="watching">Смотрю</option><option value="completed">Посмотрел</option><option value="planned">Запланировано</option><option value="on_hold">Отложено</option><option value="dropped">Бросил</option><option value="rewatching">Пересматриваю</option></select><button id="animeSync" type="button">Обновить</button></div><div class="anime-meta"><span id="animeCount"></span><span id="animeTime"></span></div><div id="animeList" class="anime-list"></div><div class="anime-pagination"><button id="animePrev" type="button">← Назад</button><span id="animePageNumber"></span><button id="animeNext" type="button">Далее →</button></div></section>`;
