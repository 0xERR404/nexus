...
const content = `${include}<div id="chatPage">${status}<div class="chat-toolbar"><div class="chat-topic-row"><select id="chatTopics" aria-label="Название чата"></select><select id="chatProvider" aria-label="Сервис"><option value="deepseek">DeepSeek</option><option value="flowmusic">FlowMusic</option></select></div><button id="chatNew" type="button">Новая</button><button id="chatRename" type="button">Название</button><button id="chatDelete" type="button" class="chat-danger">Удалить</button></div><div class="chat-meta"><select id="chatModel" aria-label="Модель DeepSeek">${modelOptions}</select><span id="chatUsage"></span><button id="flowCredits" type="button" hidden>Кредиты · —</button></div><dialog id="flowCreditDialog" aria-labelledby="flowCreditTitle"><div class="chat-title"><h2 id="flowCreditTitle">Кредиты FlowMusic</h2><button id="flowCreditClose" class="dialog-close" aria-label="Закрыть">×</button></div><p id="flowCreditStatus"></p><div id="flowCreditHistory"></div><p class="chat-hint">Изменение остатка включает действия вне хаба. Это не счёт за отдельную генерацию.</p></dialog><button id="chatOlder" type="button" hidden>Ранее</button><div id="chatMessages" aria-live="off"></div><form id="chatComposer"><label class="sr-only" for="chatInput">Сообщение</label><textarea id="chatInput" rows="2" maxlength="32000" placeholder="Напиши сообщение…" required></textarea><div class="chat-actions"><label class="chat-file" for="chatFile">Файл<input id="chatFile" type="file" accept=".txt,.md,.js,.mjs,.json,.csv,.log,.sh,.html,.css,.yaml,.yml,.ts,.py" class="sr-only"></label><span class="chat-hint">Enter — отправить · Shift+Enter — строка</span><button id="chatStop" type="button" hidden>Стоп</button><button id="chatSend" type="submit">Отправить</button></div></form><dialog id="chatDialog" aria-labelledby="chatDialogTitle"><form id="chatDialogForm"><div class="chat-title"><h2 id="chatDialogTitle"></h2>${closeButton}</div><label id="chatTitleLabel" for="chatTitle">Название<input id="chatTitle" maxlength="100" required autocomplete="off"></label><p id="chatDialogText"></p><p id="chatDialogError" class="chat-error" role="alert"></p><div class="chat-actions chat-end"><button type="button" data-chat-close id="chatCancel">Отмена</button><button type="submit" id="chatConfirm">Сохранить</button></div></form></dialog></div>`;
