...
while true; do
    ...
    if [ -f /var/lib/nexus404-base/state ]; then
        # grep -c с нулевыми совпадениями печатает "0", но код выхода 1 —
        # "|| echo 0" тогда срабатывает ДОПОЛНИТЕЛЬНО, давая "0\n0" вместо
        # "0". "|| true" + явный дефолт — как в nexus404-base-setup.sh.
        DONE=$(grep -c '^step1_' /var/lib/nexus404-base/state 2>/dev/null || true)
        DONE="${DONE:-0}"
        echo "  ${CYAN}[*]${NC} Базовая настройка: $DONE из 16 шагов"
    fi
    ...
    case "$CHOICE" in
        1)
            ...
            ;;
        2)
            # См. пояснение про grep -c у пункта с DONE= выше — тот же
            # приём (переменная + "|| true" + дефолт), не инлайн в [ ].
            BASE_STEPS_DONE=$(grep -c '^step1_' /var/lib/nexus404-base/state 2>/dev/null || true)
            BASE_STEPS_DONE="${BASE_STEPS_DONE:-0}"
            if [ ! -f /var/lib/nexus404-base/state ] || [ "$BASE_STEPS_DONE" -lt 16 ]; then
        ...
        3)
            ...
            ;;
        4)
            # Независим от пунктов 1-3 — ставится на ДРУГОЙ сервер, не
            # требует Docker/базовой настройки/самого хаба.
            echo "${CYAN}[*]${NC} Синхронизирую репозиторий..."
            git -C "$REPO_DIR" fetch -q origin
            git -C "$REPO_DIR" reset -q --hard origin/main
