...
//
// Показывает журнал событий сервера (вход, обновления, перезагрузки,
// проверки, очистка — всё, что пишет event_hook.sh + хаб) и включает
// push-уведомления — каждое событие отдельным уведомлением, не сводкой.
//
// Два источника: локальные (через хаб, GET /internal/recent-events — тот
// уже читает свой EVENTS_LOG напрямую) и удалённые (агент шлёт HTTP-
// запросом с тем же токеном, что и для метрик, хранятся в своей папке
// модуля, не в общем журнале хаба).
//
// Сама отправка push (VAPID, подписки) — на стороне хаба (push.ts),
// модуль просит "разошли вот это" через /internal/send-push.
...
async function appendRemoteEvent(entry) {
  ...
  // Раз добавили — заодно проверим, не пора ли обрезать (читаем всё равно каждый раз при показе журнала).
...
// "Очистить журнал" не трогает сам events.jsonl хаба — по нему же
// считает позицию eventWatcher.ts, его же архивирует logrotate. Вместо
// удаления строк — своя "точка отсечения": всё старше отметки просто не
// показываем в списке этого модуля.
...
// Категория/цвет по типу события — тот же список, что у push-фильтра в
// eventWatcher.ts (разные рантаймы, держим списки в согласии вручную).
function classify(type) {
  ...
  // Успешный вход по SSH — не тревожное событие, красным как у общей
  // "безопасности" выглядело бы как будто что-то не так.
...
// Понятный текст для конкретных типов событий вместо перечисления полей
// details как есть — та же логика, что в eventWatcher.ts для push-текста.
...
const server = http.createServer(async (req, res) => {
  ...
  // POST /api/clear-events — не трогает events.jsonl хаба, просто
  // отмечает точку отсечения. Обычная сессионная ручка, не про агентов.
  ...
  // POST /api/agent/report-event — тот же формат авторизации, что у
  // мониторинга (Bearer-токен, спрашиваем у хаба на каждый запрос) —
  // один и тот же агент репортит и метрики, и события.
