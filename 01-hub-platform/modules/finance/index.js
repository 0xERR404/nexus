...
//
// Четыре карточки: Карта (+/- пополнение/списание, описание обязательно),
// Курс валют (доллар/евро/тенге/юань к рублю, ЦБ РФ), Крипта (BTC/ETH/
// монеро/TON, CoinGecko), Депозит (+/- перевод с карты и обратно, без
// описания — не трата и не приход, поэтому не входит в "потрачено за
// месяц").
//
// Ниже — единая лента всех операций, новые сверху, редактирование/
// удаление аккуратно откатывают/переприменяют эффект на балансы (см.
// applyEffect), не просто правят цифру в истории саму по себе.
//
// Кнопки "+"/"-" открывают настоящий <iframe> с формой (см. GET /entry
// ниже) — тот же iframe для редактирования, с параметром edit=<id>.
//
// Курсы валют/крипты — единственные внешние HTTP-запросы напрямую (не
// через хаб): там нет секрета, оба источника публичные без ключа.
// Кэшируются на RATES_CACHE_MS, чтобы не дёргать API на каждое открытие.
...
// Применяет (sign=+1) или откатывает (sign=-1) эффект операции на балансы
// — единая точка для создания/редактирования/удаления. account='card':
// трогает только карту. account='deposit': трогает депозит на delta, а
// карту на ОБРАТНУЮ delta (пополнение депозита списывает с карты).
...
// Переводы на депозит в сумму не входят вообще — только карта, только списание.
...
// Promise.allSettled — отказ одного источника не должен прятать данные
// другого: упал CoinGecko — валюты всё равно показываем.
...
// Главная страница — 4 карточки + история. Значения не подставляются в
// разметку (устареют при открытии из кэша) — заготовки с id, клиентский
// JS подтягивает через /api/state и /api/rates.
//
// Курсы — те же плитки (.stat-tiles-grid), что у карточек на главной хаба.
//
// Порядок карточек: Карта, Депозит, Курс валют, Крипта — важен на
// мобильной сетке 2 колонки, даёт ряды "Карта+Депозит" и "Курс+Крипта".
const BODY_CONTENT = `
...
const EXTRA_SCRIPT = `
  ...
  // Высота окна зависит от поля "Описание" — оно только у карты, у
  // депозита его нет, форма короче.
  ...
  // Делегирование на весь блок — строки перерисовываются целиком при
  // каждом loadState. Карточка сама не кликабельна на просмотр — счёт/
  // описание/сумма и так видны в карточке, реагируем только на кнопки.
  ...
  // Сообщение от формы внутри iframe — операция сохранена, закрываем
  // оверлей и обновляем цифры. iframe создан нами же на свой origin,
  // посторонний контент туда попасть не может.
...
// GET /entry — форма внутри iframe (см. openEntry выше), без общей шапки.
//
// Два режима: create (пустые поля, POST) и edit (edit=<id>, поля
// заполнены, PATCH). Поле "Описание" не показывается для account='deposit'.
...
// HTTP-сервер — сырой node:http, без фреймворка, для пары ручек не нужен.
const server = http.createServer(async (req, res) => {
  ...
  // Сводка для мини-карточки на главной хаба: карта, депозит, потрачено за месяц.
  ...
  // PATCH/DELETE /api/transactions/<id> — сначала откатывают эффект в
  // старом виде, затем PATCH применяет заново с новыми значениями
  // (account/direction не меняются — проще удалить и создать заново).
  // Баланс никогда не расходится с историей.
