...
// Карта, Курс валют (ЦБ РФ), Крипта (CoinGecko), Депозит — история
// операций редактируется/удаляется через applyEffect (откат/переприменение
// эффекта на баланс). Курсы — единственные прямые внешние запросы (без
// секрета, кэш на RATES_CACHE_MS).
...
// fiat/crypto обновляются независимо и только при успехе — неудачная
// сторона оставляет последнее удачное значение, не гасит его.
...
// Значения не подставляются в разметку — заготовки с id, JS подтягивает
// через /api/state и /api/rates. Порядок карточек важен для мобильной
// сетки 2 колонки: Карта+Депозит, Курс+Крипта.
const BODY_CONTENT = `
