// NEXUS404 — модуль "Системные ресурсы". CPU/RAM/диск/сеть/аптайм:
// свой сервер (через /host/proc, /host/root — см. metrics.js) и
// удалённые (агент, см. ../../agent/). История на диске переживает
// перезапуск хаба.
...
// Имя резолвится один раз при старте (LOCAL_SERVER_NAME, иначе из
// /app/data, иначе генерируется). Переименование не убирает старую
// запись — та просто уйдёт в "оффлайн".
...
const EXTRA_SCRIPT = `
  ...
  // Имя сервера — данные, введённые снаружи (POST /api/agent/report с
  // ЛЮБОГО сервера, у которого есть общий токен агентов; локальное имя
  // тоже настраиваемое через LOCAL_SERVER_NAME). Раньше вставлялось в
  // innerHTML как есть — сервер с именем вроде "<img src=x onerror=...>"
  // выполнил бы произвольный JS в сессии браузера, открывшего эту
  // страницу (постоянный XSS, переживающий перезапуск модуля — имя
  // хранится на диске). Экранируем везде, где имя идёт в разметку.
  ...
  function renderServerCard(s) {
    var deleteBtn = s.online
      ? '<button class="icon-btn delete-server-btn" disabled title="сервер ещё активен — удаление бессмысленно, отчёт придёт заново через пару секунд" style="margin-left:auto; opacity:0.35; cursor:not-allowed;">' +
        ...
        '</button>'
      : '<button class="icon-btn delete-server-btn" data-name="' + escapeHtml(s.name) + '" title="удалить из списка" style="margin-left:auto;">' +
          '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>' +
        '</button>';
    ...
    return '<div class="server-card" data-server-card="' + escapeHtml(s.name) + '">' + head + grid + uptime + '</div>';
  }

  // Обновляет ТОЛЬКО текст внутри уже существующей карточки — полная
  // пересборка через renderServerCard нужна лишь когда меняется состав
  // серверов, не когда подъехали новые цифры (иначе карточка визуально
  // "дёргалась" бы каждую секунду).
  ...
  // Подгоняет разрешение canvas под devicePixelRatio — иначе на retina
  // картинка смазанная. Координаты в drawChart остаются в CSS-пикселях.
  ...
  function drawChart(canvas, seriesList, maxOverride, unit) {
    ...
    // Подписи — после линий данных, иначе линия могла бы их перекрыть.
  ...
  // Пересчёт разрешения canvas при ресайзе окна (поворот телефона,
  // изменение окна на десктопе) — setupCanvas сам читает актуальный
  // clientWidth, здесь просто вызываем loadGraphs заново, с debounce.
...
const server = http.createServer(async (req, res) => {
  ...
  // DELETE /api/servers/:name — стирает и запись, и историю на диске.
  // Проверка "не активен ли" на бэкенде, не только в интерфейсе — если
  // сервер отчитывается, удаление тут же откатится следующим тиком.
  ...
  // POST /api/agent/report — единственная ручка без сессии браузера (хаб
  // пропускает публично). Вместо сессии — общий токен агентов,
  // спрашиваем у хаба на каждый запрос, не кэшируем.
