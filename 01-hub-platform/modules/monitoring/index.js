...
// Имя сервера резолвится ОДИН раз при старте процесса (LOCAL_SERVER_NAME,
// иначе сохранённое в /app/data, иначе генерируется заново), не на
// каждый тик. Смена имени — только пересозданием контейнера.
// Честная оговорка: переименование не убирает старую запись из списка —
// та просто перестанет обновляться и уйдёт в "оффлайн" через OFFLINE_AFTER_MS.
...
const EXTRA_SCRIPT = `
  ...
  // Готовит canvas: подгоняет внутреннее разрешение под реальный
  // CSS-размер с учётом devicePixelRatio — иначе на широких экранах и
  // retina-дисплеях картинка смазанная и растянутая непропорционально.
  // Контекст масштабируется обратно, координаты внутри drawChart
