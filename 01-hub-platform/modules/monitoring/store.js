...
// /app/data переживает docker rm -f. Текущие значения — только в памяти
// (обновятся в первый тик), на диск пишем только историю для графиков.
...
// 5с — с запасом ниже OFFLINE_AFTER_MS (15с). Порог не привязан к
// SAMPLE_INTERVAL_MS — агенты шлют реже, иначе всегда были бы "оффлайн".
...
// генерируем один раз и сохраняем в /app/data (переживает docker rm -f),
// не os.hostname() — тот меняется при каждом пересоздании контейнера.
