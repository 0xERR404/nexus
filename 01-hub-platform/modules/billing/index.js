...
// Claude): баланс (только у DeepSeek есть публичный эндпоинт баланса —
// у остальных честно "не поддерживается") и токены за час/сутки/месяц/
// всё время (скользящие окна, не календарные — см. usage.ts). Сами
// ключи сюда не попадают, только готовый результат от хаба.
//
// Точка у баланса — красная/зелёная строго по тому, задан ли ключ, не
// по тому, удалось ли реально его получить. Статус ключей хаб отдаёт
// браузеру напрямую через GET /api/settings/keys (та же сессионная
// кука) — отдельный internal-эндпоинт не нужен.
...
// Собирает всё для страницы и мини-карточки одним запросом — usage
...
// Общий "плиточный" вид: подпись сверху, число снизу — короткая пара
// "подпись + значение" на тесной ширине переносится непредсказуемо и
// меняет высоту. Тот же класс использует и мини-карточка на главной.
const TOTALS_ROW = `
...
const EXTRA_SCRIPT = `
  ...
  async function refresh() {
    try {
      // Относительный путь без ведущего слэша — страница открыта на
      // /modules/billing/, с ведущим слэшем запрос улетел бы в корень
      // хаба. Статус ключей — напрямую у хаба, с ведущим слэшем (тот
      // путь и живёт в корне).
      ...
      providers.forEach(function (id) {
        ...
        // У Gemini точка отражает последний реально увиденный статус —
        // красная при лимите/проблеме с биллингом, даже если ключ на
        // месте. Остальные провайдеры — просто "задан/не задан".
        ...
        if (id === 'deepseek') {
          ...
          // Не баланс (у Gemini через обычный ключ его не получить —
