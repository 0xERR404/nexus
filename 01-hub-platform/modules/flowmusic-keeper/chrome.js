// NEXUS404 — общий каркас страницы модуля. Один файл вместо копирования
// CSS/разметки шапки в каждом — модули вызывают renderPage(...) и
// добавляют свой контент. Dockerfile копирует его рядом с index.js.
// Шапка идентична шапке хаба (dashboard.ts) — тот же prompt, "online",
// кнопка "выйти".
...
const HEADER_SCRIPT = `
  // Толчок скролла на 1px и обратно — мобильные браузеры сворачивают
  // часть интерфейса при первом скролле, меняя видимую высоту страницы.
  // Возвращаем свою текущую позицию, не жёсткий 0 — не перебивает
  // восстановленную браузером прокрутку.
  ...
  // Замена системного confirm() браузера в стилистике приложения.
  // window.nexusConfirm(message, opts) -> Promise<boolean>. opts.danger —
...
// title — заголовок модуля, bodyContent/extraScript — контент и JS
// модуля, username — для prompt в шапке. escapeHtml — тот же принцип,
// что и в hub/src/dashboard.ts, для защиты от будущих небезопасных
// источников title/username, даже если сегодня оба доверенные.
