...
const paths = {
  ...
  // Refresh-токен сессии steam-user — НЕ пароль (тот нигде не
  // сохраняется, вводится один раз интерактивно через setup-скрипт и
  // держится только в памяти процесса на время логина). Токен сам по
  // себе достаточно чувствителен (даёт вход без 2FA заново) — файл
  // создаётся с правами 0600, см. lib/steamClientApi.js.
  steamSessionFile: path.join(DATA_DIR, 'steam_client_session.json'),
};
...
<style>
  ...
  /* Ровно тот же размер, что у "← Назад" в шапке модуля (.back-btn в
     chrome.js): padding 5px 12px, border-radius 4px, font-size 0.8rem —
     раньше кнопки обновления и вкладки были крупнее и разного размера
     друг с другом. */
  ...
  .cs-tab-btn svg { width: 13px; height: 13px; }
  ...
  /* Без этого правила #ach-modal-body был обычным блоком без display:flex —
     flex:1/min-height:0 у .cs-ach-modal-list внутри него ничего не значили
     (flex-свойства работают только у прямого ребёнка flex-контейнера), и
     список просто растягивался под всё содержимое (проверено: 2392px тела
     при ограничении бокса в 674px), вместо того чтобы прокручиваться в
     оставшемся месте — самой прокрутки не было вообще, не только "плохо
     видна". */
  #ach-modal-body { display: flex; flex-direction: column; flex: 1; min-height: 0; }
  .cs-ach-modal-list { display: flex; flex-direction: column; gap: 8px; overflow-y: auto; flex: 1; min-height: 0;
    /* Без этого при докрутке списка до конца колесо/свайп "утекает"
       дальше и начинает крутить страницу ПОД модалкой — visуально
       выглядит как "сначала крутится список, потом уже фон". JS ниже
       (openAchievementsModal/closeAchievementsModal) блокирует скролл
       фона полностью, это — вторая линия защиты именно для скролла
       ВНУТРИ списка, не даёт ему передать остаток жеста родителю. */
  ...
  /* Рамка была фиксированно зелёной для ЛЮБОЙ открытой ачивки — при том,
     что у нас уже есть шкала редкости (см. rarityColorFor, RARITY_TIER_META)
     и она же красит галочку/процент. Рамка её игнорировала — "счётчик
     крутости" за окном, а карточки все одного цвета. Теперь цвет рамки —
     инлайн-стиль per-row, та же rarityColorFor, что и у галочки/процента;
     .unlocked ниже остаётся как fallback на случай globalPercent===null. */
  ...
  /* Цвет самой галочки теперь задаётся инлайн по редкости конкретной
     ачивки (см. renderAchievementsModal — rarityColorFor), не плоским
     зелёным для всех подряд открытых. */
  ...
  @media (max-width: 600px) {
    ...
    .cs-tab-btn { flex: 1 1 auto; justify-content: center; }
...
// Тиры "крутости" ачивок по мировой редкости — порядок, границы и цвета
// должны совпадать с RARITY_TIERS в lib/stats.js.
const RARITY_TIER_META = [
  { id: 'gold',   ceiling: 1.0,   label: 'Легендарные', color: '#dcb35c' },
  { id: 'purple', ceiling: 3.0,   label: 'Эпические',   color: '#b478f0' },
  { id: 'blue',   ceiling: 8.0,   label: 'Редкие',      color: '#4a90d9' },
  { id: 'green',  ceiling: 20.0,  label: 'Необычные',   color: '#4cb389' },
  { id: 'white',  ceiling: 50.0,  label: 'Обычные',     color: '#d7dbe0' },
  { id: 'gray',   ceiling: 101.0, label: 'Частые',      color: '#6b7280' },
];

// Цвет одной ачивки по её мировой редкости — та же шкала, что и у
// "Счётчика крутости"/"Самых редких" на этой же странице, только для
// каждой отдельной ачивки в модалке, не только для сводных тиров.
function rarityColorFor(percent){
  if(percent === null || percent === undefined) return 'var(--muted)';
  for(const t of RARITY_TIER_META){
    if(percent <= t.ceiling) return t.color;
  }
  return RARITY_TIER_META[RARITY_TIER_META.length - 1].color;
}
...
function updateStats(s){
  ...
  // Разбивка по источникам — видно наведением, не занимает места на
  // странице лишним баннером. Если реальных игр больше, чем в сумме —
  // почти наверняка Family Sharing (одолженные из чужой библиотеки):
  // ни GetOwnedGames, ни публичный XML-фид профиля их принципиально не
  // видят, Steam-клиент их показывает только потому, что видит
  // объединённую библиотеку. Чинится не кодом, а вручную через
  // manual_appids.json (см. комментарий в lib/stats.js).
  const lc = s.libraryCheck;
  if(lc && typeof lc.ownedGamesCount === 'number'){
    const sourceLabel = lc.baseSource === 'html' ? 'HTML-страница профиля' : lc.baseSource === 'xml' ? 'XML-фид профиля (резерв)' : 'только GetOwnedGames — профиль недоступен';
    const ownedOnlyPart = lc.ownedOnlyCount ? \`, из них \${lc.ownedOnlyCount} нашёл только GetOwnedGames\` : '';
    const extraPart = lc.extraGamesCount ? \` + \${lc.extraGamesCount} через недавнюю активность/manual_appids.json\` : '';
    document.getElementById('val-games-count').title =
      \`База: \${lc.baseCount} игр (источник: \${sourceLabel})\${ownedOnlyPart}\${extraPart}. Если реальных игр больше — см. manual_appids.json: игры через Family Sharing ни один официальный Steam источник не отдаёт вообще, добавляются только вручную.\`;
  }
  ...
  // GetOwnedGames иногда не видит часть игр (F2P без запуска через
  // клиент, семейный доступ и т.п.) — сверка через публичный XML-фид
  // профиля (steamcommunity.com/profiles/.../games?xml=1) добирает их.
  // Раньше её успех/провал был виден только в логах хаба (logger.warn),
  // на странице — тишина, даже если счётчик игр меньше реального.
  //
  // ВАЖНО (было реальной ложной тревогой): "не нашла ни одной
  // дополнительной игры" САМО ПО СЕБЕ — это НЕ проблема, а норма для
  // большинства аккаунтов (GetOwnedGames и так видит всё). Предупреждение
  // ниже показывается ТОЛЬКО если Steam явно заблокировал анонимный
  // доступ (privacyBlocked=true, обычно — редирект на /login) — не
  // просто "нашлось 0". ВАЖНО: несмотря на название поля, это НЕ всегда
  // настройка приватности буквально — на реальном аккаунте она была
  // открыта, а редирект всё равно случился, причина не установлена. Не
  // утверждай в тексте предупреждения конкретную причину, которую не
  // можешь подтвердить.
...
// Блокировка скролла ФОНА, пока модалка открыта — не просто
// overflow:hidden на body (в мобильном Safari он не всегда держит,
// палец всё равно может утащить страницу под модалкой при быстром
// свайпе) — фиксируем body на текущей позиции скролла и восстанавливаем
// её же при закрытии. Без этого было ровно то, что и жаловались:
// сначала докручивается список ВНУТРИ модалки, а затем то же самое
// движение "перетекает" и начинает крутить страницу позади неё.
let savedScrollY = 0;
...
function renderAchievementsModal(data, platform){
  ...
  // Личный прогресс (что именно выбито) не удалось получить у Steam
  // после ретраев — раньше это просто молчало и показывало ВСЁ как
  // "не выбито", неотличимо от реальной честной ситуации "нет вообще
  // ничего". У игры может быть 100% выполнено, а модалка покажет
  // сплошные серые галочки без единого слова объяснения почему.
  const warningHtml = data.personalDataUnavailable
    ? \`<div class="cs-modal-empty" style="background:rgba(239,83,80,0.1);border:1px solid rgba(239,83,80,0.3);border-radius:var(--card-radius);padding:9px 14px;margin-bottom:10px;color:#f5b3b7;font-size:12px;text-align:left;">⚠ Не удалось получить ТВОЙ личный прогресс по этой игре у Steam (сеть/лимит после нескольких попыток) — ниже список ачивок сам по себе верный, но пометки "выбито" сейчас недостоверны. Обнови ещё раз через минуту.</div>\`
    : '';
  body.innerHTML = warningHtml + \`<div class="cs-ach-modal-list">\${data.achievements.map(a => {
    ...
    const rarityColor = rarityColorFor(a.globalPercent);
    // Цвет галочки по редкости — только у ОТКРЫТЫХ (нечего праздновать
    // редкость того, чего ещё нет): золото за легендарную, серый за частую.
    // Закрытые остаются нейтрально-серыми, как и раньше.
    const checkStyle = unlocked ? \`background:\${rarityColor}26; color:\${rarityColor};\` : '';
    // Рамка карточки — та же логика, что и у галочки: цвет по редкости
    // только у открытых (нечего подсвечивать редкость того, чего ещё нет).
    // globalPercent===null (rarityColorFor вернёт var(--muted)) — намеренно
    // не подсвечиваем инлайном, отдаём это дефолтному .unlocked ниже
    // (зафиксированный зелёный) — так понятнее "выбито, но редкость
    // неизвестна", чем частокол из одинаковых блёкло-серых рамок.
    const rowBorderStyle = (unlocked && a.globalPercent !== null && a.globalPercent !== undefined)
      ? \`style="border-color:\${rarityColor}59"\`
      : '';
    return \`
    <div class="cs-ach-modal-row \${unlocked ? 'unlocked' : ''}" \${rowBorderStyle}>
      <div class="cs-ach-check" style="\${checkStyle}">\${unlocked ? ICONS.check : ''}</div>
      ...
      <div class="cs-ach-rarity" style="color:\${rarityColor}">\${rarityText}</div>
    </div>\`;
  }).join('')}</div>\`;
}
