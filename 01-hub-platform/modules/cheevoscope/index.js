...
// Ключи — своя ручка GET /internal/cheevoscope-keys, настраиваются в
// "AI API", спрашиваются заново перед каждым прогоном пайплайна, не
// кэшируются. Своей авторизации нет — сессионная кука хаба, как и везде.
// Почасовая автопроверка — обычный setInterval, не systemd-таймер.
...
const paths = {
  ...
  // Refresh-токен сессии steam-user — не пароль, но чувствителен (даёт
  // вход без 2FA), файл 0600 (см. lib/steamClientApi.js).
...
// Если вход через Steam уже выполнен (пароль/QR), SteamID64 известен
// из сохранённой сессии — приоритет у неё; ручное поле steamId в
// настройках остаётся для обратной совместимости.
...
// Свежие ключи на каждый запрос модалки, тот же принцип, что у getStats
// выше — используем resolveConfiguredSteamId(), не keys.steamId напрямую
// (тот пуст для вошедших через Steam-логин).
...
const EXTRA_SCRIPT = `
// Рендеринг Steam-отчёта — точечные обновления DOM (фронт дёргает
// /api/status раз в 2с). Skeleton строится один раз, дальше — только
// правки текста/стилей существующих узлов. Плитки — в tileMap по appid.
...
function updateStats(s){
  ...
  // Если реальных игр больше суммы источников — почти наверняка Family
  // Sharing, чинится вручную через manual_appids.json (см. lib/stats.js).
  ...
  // XML-фид профиля добирает игры, которые GetOwnedGames не видит.
  // "Нашлось 0" — норма для большинства аккаунтов, не проблема;
  // предупреждение — только при явном privacyBlocked=true (редирект на
  // /login), причину не утверждаем конкретно (не всегда буквально приватность).
...
function filterGamesGrid(gridId, searchId, sortId){
  ...
  if(sortBy !== 'name'){
    ...
    // Восстанавливаем алфавитный порядок — сортировка по часам/%
    // физически переставляет плитки в DOM, простой чинилки видимости
    // подписей недостаточно, нужна реальная пересортировка.
    if(labels.length){
...
// Модалка закрывается по backspace/кнопке "назад" — history.pushState
// при открытии, физическая кнопка генерирует popstate, не keydown.
let modalHistoryPushed = false;

// Блокировка скролла фона — не просто overflow:hidden (мобильный Safari
// не всегда держит), фиксируем body на текущей позиции и восстанавливаем.
let savedScrollY = 0;
...
// fromPopstate=true — закрыла кнопка "назад", запись истории уже
// израсходована браузером. false — закрыли крестиком/backspace, запись
// убираем сами.
...
function renderAchievementsModal(data, platform){
  ...
  // Личный прогресс не получен после ретраев — показываем предупреждение,
  // не молчим (иначе 100%-пройденная игра выглядела бы сплошь невыбитой).
  ...
  body.innerHTML = warningHtml + \`<div class="cs-ach-modal-list">\${data.achievements.map(a => {
    ...
    // Цвет рамки по редкости только у открытых; null-редкость отдаём
    // дефолтному .unlocked (зелёный), не блёкло-серому инлайну.
...
(function initSwipeNav(){
  ...
  // rAF-троттлинг — touchmove стреляет чаще экрана, лишние mutation
  // стиля между кадрами только добавляют лаг на тяжёлых панелях.
...
const server = http.createServer(async (req, res) => {
  ...
  // Веб-альтернатива CLI-скрипту lib/steamClientAuth.js. Пароль не
  // участвует — сканируется QR мобильным Steam, мы только показываем
  // картинку и ждём результата.
