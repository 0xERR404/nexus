...
// Вход паролем через HTTP — тот же механизм, что и в
// steamClientApi.js/runInteractiveLogin() (интерактивный CLI-вариант,
// который остаётся рабочим и полезным для отладки через docker exec),
// но адаптирован под веб-форму: логин/пароль/код Guard приходят по
// одному HTTP-запросу за раз, а не читаются интерактивно из stdin.
// Пароль НИГДЕ не сохраняется — участвует только в вызове logOn(),
// живёт в памяти ровно на время самого запроса.
