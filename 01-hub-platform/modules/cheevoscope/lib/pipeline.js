...
function createPipeline({ getStats, statusFile, cacheDir, logger = console }) {
  ...
  //
  // Картинки (витрина) и достижения (Web API) — те же два независимых
  // лимитера/домена, что и в runFullPipeline ниже (см. подробное
  // объяснение там) — запускаем параллельно, не строго друг за другом.
  // Достижения пишут прогресс во ВТОРОЙ трек (secondaryProgressCb), по
  // той же причине, что и в runFullPipeline: у них на порядок больше
  // отдельных запросов, без разделения полей их частые записи почти
  // всегда перетирали бы прогресс картинок в статусе.
  async function runQuickPipeline() {
    ...
    const achievementsPromise = (async () => {
      // Единственная разница с "друг за другом" версией: если картинок
      // для новых игр нет (newGames.length === 0), эта ветка стартует
      // не позже, а РАНЬШЕ, чем раньше — прежний код сначала ждал бы
      // условие if(newGames.length), даже когда оно false, прежде чем
      // дойти сюда; теперь обе ветки запускаются сразу одним and-then.
