// RA-аналог stats.js. Список игр — один вызов (API_GetUserCompletionProgress,
// постранично), не перебор appid. Per-game вызов нужен только для Points
// в "полном" режиме, заодно прогревает кэш модалки (achievementDetails.js).
...
function createRetroStats({ retroApi, cache, paths, computeRarityTiers, logger = console }) {
  ...
  // GetGameInfoAndUserProgress для реальных Points. Кэш под retro_detail_{gameId}
  // — тот же, что читает модалка, открывается мгновенно после "Обновить всё".
  ...
  async function generateReport(profile, progressList, awards, recent, gameDetails) {
    ...
    for (const entry of progressList) {
      ...
      // Тот же фильтр, что в achievementDetails.js — null/примитив среди
      // ачивок валил бы .filter/.reduce ниже, обрывая весь отчёт.
      const detailAchievements = Object.values(detail.Achievements || {}).filter((a) => a && typeof a === 'object');
