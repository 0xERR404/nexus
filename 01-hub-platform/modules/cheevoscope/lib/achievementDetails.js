...
function createAchievementDetails({ steamApi, retroApi, cache, logger = console }) {
  ...
  async function getSteamGameAchievements(appid) {
    ...
    // Личный прогресс не удалось получить (после всех ретраев) — это
    // видно и в логе (выше), и в самом ответе (personalDataUnavailable),
    // чтобы при необходимости фронтенд мог отличить "точно не открыто" от
    // "не удалось проверить" — сейчас это только диагностика, поведение
    // модалки пока не меняет (см. личный прогресс = [] в обоих случаях).
