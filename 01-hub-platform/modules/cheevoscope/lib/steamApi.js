...
// у неё гораздо более жёсткий, НЕофициальный (по опыту сообщества, Valve
// не публикует точных цифр) rate-limit — порядка 200 запросов/5 минут на
// IP. 0.6с (~1.67 запроса/сек) — сознательно ближе к этому пределу, чем
// было (0.8с), но не вплотную: наступить на реальный лимит здесь выйдет
// дороже, чем выиграть — 429 с бэкоффом отменит всю экономию и рискует
// временно забанить IP от витрины целиком.
...
function createSteamApi({ apiKey, steamId, logger = console }) {
  ...
  // steamId из конфига может быть готовым SteamID64, vanity-именем или
  // ссылкой на профиль (см. resolveSteamId64 ниже) — но большинству
  // ручек Steam Web API (GetOwnedGames, GetRecentlyPlayedGames,
  // GetPlayerAchievements) нужен именно числовой SteamID64, текст они
  // не резолвят сами и отвечают HTTP 400. Раньше resolveSteamId64()
  // вызывался ТОЛЬКО из отдельного резервного пути (полный список
  // библиотеки через XML-фид, см. stats.js) — во все остальные запросы
  // steamId уходил как есть. Если в конфиге указано что угодно, кроме
  // готового 17-значного ID (например, ссылка на профиль или ник) —
  // ПЕРВЫЙ ЖЕ вызов getOwnedGames() валился с HTTP 400 и ронял весь
  // пайплайн целиком, ещё до того, как дело доходило до отдельных игр.
  // Резолвим один раз и кэшируем — все внутренние вызовы ниже используют
  // getResolvedSteamId(), а не сырой steamId напрямую.
  ...
  // не отдаёт). ПРЕДПОЛАГАЛОСЬ, что не требует авторизации при открытой
  // настройке "Сведения об играх" — но это опровергнуто на реальном
  // аккаунте: настройка стоит "Открытый" (проверено владельцем
  // напрямую в Steam), и Steam всё равно перенаправляет анонимный
  // запрос на /login. Реальная причина такого поведения НЕ установлена
  // — важно не приписывать её конкретной настройке приватности, раз
  // это уже один раз оказалось неверным.
  //
  // Возвращает { games, privacyBlocked }, а не просто массив — вызывающей
  // стороне ВАЖНО различать "прогнали фид, реально нечего добавлять"
  // (нормальный исход для большинства аккаунтов — GetOwnedGames и так
  // видит всё) от "Steam заблокировал анонимный доступ" (это как раз
  // стоит показать пользователю, не привязывая к конкретной причине).
  // Название поля privacyBlocked сохранено ради обратной совместимости
  // остального кода — семантика "доступ заблокирован", не буквально
  // "дело в настройке приватности".
  async function getFullLibrary(steamid64) {
    ...
    // Реально пойманный случай (не предположение): Steam на приватный
    // "Сведения об играх" отвечает НЕ ошибкой в теле XML, а 302-редиректом
    // на /login/?redir=... — fetch() по умолчанию следует за редиректом
    // сам, так что тело ответа тут — HTML-страница логина, не XML вообще.
    // Без этой проверки такой случай молча схлопывался в "пустой список",
    // неотличимо от "и так всё нашли, добавлять нечего".
  ...
  // Тот же публичный список игр, что и getFullLibrary() выше, но через
  // СОВРЕМЕННУЮ HTML-страницу (steamcommunity.com/profiles/ID/games/?tab=all
  // — ровно та же страница, что открывается в браузере), а не через
  // xml=1-фид. Возвращает { games, privacyBlocked } — тот же контракт,
  // что и у getFullLibrary(), по той же причине (см. там).
  async function getFullLibraryHtml(steamid64) {
    ...
    // Реально пойманный случай (curl без -L показал это буквально):
    // Steam на приватные "Сведения об играх" отвечает 302 на
    // /login/?redir=... — fetch() следует за редиректом сам, так что
    // здесь в html лежит страница логина, а не список игр. См. тот же
    // комментарий в getFullLibrary() выше — тот же диагноз, тот же
    // признак (res.url после fetch содержит /login).
    if (res.url && res.url.includes('/login')) {
    ...
    if (!match) {
      // Профиль публичен (редиректа на логин не было), но ожидаемого
      // блока в разметке нет — либо Valve поменяла формат страницы,
      // либо у этого конкретного профиля сейчас 0 игр. Не считаем
      // ошибкой приватности — тем более раз мы уже точно знаем, что
      // редиректа не было.
  ...
  // английский — ТАК ОБЫЧНО, но не всегда: у части игр (обычно инди,
  // без официального русского вообще — например, без переведённых
  // текстов ачивок разработчиком) Steam на l=russian отдаёт пустой
  // achievements[] целиком, хотя у игры их сотни (воспроизведено:
  // "100% Orange Juice" — валится в "Нет данных о достижениях",
  // хотя реально 300+ ачивок). Раньше пустой ответ на РУССКОМ запросе
  // считался окончательным "у игры нет ачивок" — неверно, это не
  // отсутствие ачивок, а отсутствие ИМЕННО русской локализации для
  // них. Если RU-запрос вернул пусто — переспрашиваем без l= вообще
  // (Steam отдаёт дефолтный язык схемы, обычно английский) прежде чем
  // считать игру окончательно без ачивок.
