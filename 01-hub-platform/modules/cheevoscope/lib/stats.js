...
function createStats({ steamApi, cache, paths, apiConcurrency = 10, storeConcurrency = 5, logger = console }) {
  ...
  // 21 день (не 7) — на большой библиотеке большинство повторных
  // "Обновить всё" попадает в кэш, не в самый медленный источник (витрину).
  ...
  // Собирает полный список игр. Первичный источник — страница профиля
  // (games/?tab=all, полнее GetOwnedGames для части аккаунтов);
  // GetOwnedGames — обогащение (playtime_forever). GetRecentlyPlayedGames
  // видит игру только в окне ~2 недели — предыдущий сохранённый список
  // читается первым как база, чтобы appid, однажды увиденный любым
  // источником, не терялся при выпадении из этого окна.
  async function fetchGamesList() {
    ...
    // История — первый источник; последующие могут обновить запись
    // (.set() по appid), но не убрать её.
    try {
    ...
    // Вход как владелец аккаунта — источник высшего приоритета,
    // единственный без фильтра "непроверенных" игр (см. steamClientApi.js).
    // Сессия не настроена — needsLogin:true, источник просто пропускается.
    ...
    try {
      ...
      // .set() не должен затирать playtime_forever, уже известный из
      // истории/клиента — профиль не отдаёт время игры вообще.
    ...
    // GetOwnedGames обогащает playtime и подстраховывает, если база из
    // профиля пуста (недоступен/приватен) — деградация плавная.
    ...
    try {
      ...
      for (const g of owned) {
        const existing = known.get(g.appid);
        if (existing) {
          // Math.max, не перезапись — playtime_forever не может
          // уменьшиться со временем, max() безопасен для любого порядка
          // источников.
    ...
    // Резервные добавки — только то, чего не было нигде. _unverified:
    // true означает "подтверждено только этим слабым источником" — влияет
    // на кэш ачивок ниже (fetchOneGameAchievements).
    ...
    // Предупреждение — только если Steam явно ответил ошибкой приватности
    // (privacyBlocked=true), не просто "профиль ничего не добавил". Если
    // клиентский вход уже успешно отработал, редирект на логин у резервного
    // анонимного пути — чистый шум, не показываем.
