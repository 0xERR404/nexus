...
// Вход через клиентский протокол Steam (тот же, которым пользуется сам
// десктопный Steam-клиент) — НЕ дублирует IPlayerService/GetOwnedGames
// зря. getUserOwnedApps() внутри steam-user дёргает ТУ ЖЕ службу
// (Player.GetOwnedGames#1, см. node_modules/steam-user/components/
// friends.js) — это подтверждено чтением исходников библиотеки, не
// предположение. Разница — ОДИН конкретный параметр,
// skip_unvetted_apps, которого нет в публичном REST API вообще
// (подтверждено самой protobuf-схемой Steam: steammessages_player.
// steamclient.proto, "if set, skip unvetted store apps", default=true).
// Публичный REST-эндпоинт этот параметр никак не выставляет — значит
// всегда фильтрует "непроверенные" игры молча. Раз он выставляется
// только через авторизованный клиентский протокол — путь входа нужен
// не потому, что "видит больше в принципе", а потому, что даёт доступ
// именно к этому переключателю.
//
// ЧЕСТНО: это НЕ решает Family Sharing (тот управляется лицензиями
// аккаунта, не статусом "проверенности" игры) и не гарантирует, что
// "Crash Position" из переписки — именно unvetted-игра, а не что-то
// другое. Это конкретный, проверенный по исходникам механизм, а не
// обещание "теперь видно всё".
...
// Интерактивный одноразовый вход — запускается вручную оператором
// (docker exec -it ... node lib/steamClientAuth.js), НЕ частью
// автоматического пайплайна. Пароль живёт только в памяти этого
// процесса на время логина, нигде не сохраняется. Сохраняется только
// refreshToken — им можно войти снова без пароля, но НЕ без знания
// самого токена (файл 0600, наружу через API никогда не отдаётся).
