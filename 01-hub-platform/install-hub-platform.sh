...
if is_done "hub_4b_bridge"; then
    ...
    # Node.js на хосте — раньше был нужен только внутри Docker.
    # Формат репозитория NodeSource (deb822 .sources) сверен с рабочей
    # конфигурацией, но ветка node_20.x живым запросом не проверена —
    # при первом реальном развёртывании перепроверить вывод echo ниже.
    if ! command -v node >/dev/null 2>&1; then
        NODE_MAJOR=20
        # Тот же приём, что и для Docker выше: сохраняем ключ как есть
        # (armor, не бинарный) и ссылаемся на него через Signed-By — без
        # отдельного вызова gpg --dearmor. Раньше он был здесь и на части
        # серверов падал с "gpg: cannot open '/dev/tty'": gpg пытается
        # достучаться до терминала, которого просто нет у setsid-процесса
        # (run_spinner всегда его отвязывает) — по крайней мере на части
        # окружений, где-то это не всплывает. Раз шаг Docker выше с точно
        # таким же подходом (armor-ключ напрямую в Signed-By/signed-by=)
        # у пользователей уже отрабатывает надёжно — используем его и
        # здесь, а не чиним gpg под конкретное окружение.
        run_spinner "Загрузка ключа NodeSource" \
            "mkdir -p /usr/share/keyrings && curl -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key -o /usr/share/keyrings/nodesource.asc"
        chmod a+r /usr/share/keyrings/nodesource.asc
...
EOF
        retry_apt "Обновление списка пакетов (NodeSource)" "apt-get update"
        retry_apt "Установка Node.js ${NODE_MAJOR}.x" "apt-get install -y nodejs"
    fi
    echo "${GREEN}[✓]${NC} Node.js: $(node --version)"

    mkdir -p "$NEXUS_DIR/host-bridge"
    cp -r "$SCRIPT_DIR/host-bridge/src" "$NEXUS_DIR/host-bridge/src"
    cp "$SCRIPT_DIR/host-bridge/package.json" "$NEXUS_DIR/host-bridge/package.json"
    cp "$SCRIPT_DIR/host-bridge/tsconfig.json" "$NEXUS_DIR/host-bridge/tsconfig.json"
    (cd "$NEXUS_DIR/host-bridge" && npm install >> "$LOGFILE" 2>&1 && npm run build >> "$LOGFILE" 2>&1)
    if [ ! -f "$NEXUS_DIR/host-bridge/dist/bridge.js" ]; then
        echo "${RED}[!]${NC} Сборка host-bridge (tsc) не удалась — подробности в $LOGFILE"
        exit 1
    fi
    chmod 700 "$NEXUS_DIR/host-bridge/dist/bridge.js"
    ...
    sleep 1
    # Реальный путь сокета — /opt/nexus404/host-bridge-sock/host-bridge.sock
    # (см. SOCKET_PATH в host-bridge/src/bridge.ts), не "$NEXUS_DIR/host-bridge.sock".
    if [ -S "/opt/nexus404/host-bridge-sock/host-bridge.sock" ]; then
        echo "${GREEN}[✓]${NC} host-bridge слушает /opt/nexus404/host-bridge-sock/host-bridge.sock"
    else
...
if is_done "hub_5_up"; then
    ...
    # Без set -e голый `cd` при ошибке (например, $NEXUS_DIR внезапно
    # пропал) не остановил бы скрипт — docker compose ниже выполнился
    # бы в ТЕКУЩЕЙ директории вместо нужной, молча и без понятной причины.
    cd "$NEXUS_DIR" || { echo "${RED}[!]${NC} Не удалось перейти в $NEXUS_DIR"; exit 1; }
...
echo ""
echo "${YELLOW}[?]${NC} Caddyfile сейчас выпускает БОЕВОЙ сертификат Let's Encrypt с первого"
echo "    раза (staging CA убран из Caddyfile — см. комментарий в начале файла)."
echo "    DNS домена должен указывать на этот сервер ДО запуска, иначе Caddy"
echo "    не сможет подтвердить владение доменом — а частые неудачные попытки"
echo "    расходуют БОЕВОЙ лимит Let's Encrypt (5 сертификатов на домен в"
echo "    неделю), не staging-лимит."
echo ""
