...
# shellcheck disable=SC2154 # "code" присваивается в первой же строке тела trap ниже — ложное срабатывание
trap '
    code=$?
    if [ "$code" -ne 0 ]; then
...
if [ -f /etc/systemd/system/nexus404-host-bridge.service ]; then
    if ! command -v node >/dev/null 2>&1; then
        echo "${RED}[!]${NC} node не найден на хосте, хотя host-bridge.service уже существует —"
        echo "    похоже, host-bridge ставился ДО перехода на TypeScript. Прогони заново:"
        echo "    sudo bash install-hub-platform.sh"
        exit 1
    fi
    rm -rf "$NEXUS_DIR/host-bridge/src" "$NEXUS_DIR/host-bridge/dist"
