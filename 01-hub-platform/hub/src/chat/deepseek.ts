...
// Раньше было две модели (Flash/Pro), выбор — свойство сообщения, не
// темы. Pro убрали за ненадобностью — остался массив/тип из одного
// элемента, а не просто строковая константа, чтобы resolveDeepSeekModel
// ниже по-прежнему грациозно откатывался на дефолт, если старая вкладка
// в браузере (ещё не обновившая код) всё же пришлёт "deepseek-v4-pro" —
// не отправляем в API произвольную строку от клиента как есть.
