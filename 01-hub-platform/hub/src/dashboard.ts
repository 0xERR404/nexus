...
const MODULES_SCRIPT = `
  ...
  async function loadModules() {
    ...
    try {
      ...
      // chatCardHtml выше). Пустой список модулей — не обязательно
      // поломка: сразу после перезагрузки сервера супервизор может ещё
      // не успеть просканировать (ждёт готовности Docker, см. hostBridge.ts).
      // Пустая область без единого слова выглядела бы как крах — честное
      // сообщение вместо тишины, само пропадёт на следующем опросе
      // (раз в 10с), как только модули появятся.
...
(function () {
  ...
  function addBlock(role, text, opts) {
    ...
    if (role === 'ты') {
      ...
      // Для DeepSeek — ещё и какая именно модель ответила, сохранённое
      // значение, не текущее положение (переключателя больше нет — Pro
      // убрали, выбор стал не из чего делать). 'deepseek-v4-pro' в карте
      // оставлен намеренно, не для выбора — только чтобы у СТАРЫХ
      // сообщений в истории (отвечал ещё Pro, до этой правки) подпись
      // "· PRO" не пропадала задним числом.
  ...
  async function sendCurrentMessage() {
    ...
    // model раньше выбирался отдельным селектором (Flash/Pro) и уходил
    // сюда для тем DeepSeek. Pro убрали, выбирать стало не из чего —
    // сервер сам подставляет DEFAULT_DEEPSEEK_MODEL (flash), если body.model
    // не передан (см. resolveDeepSeekModel в chat/deepseek.ts).
