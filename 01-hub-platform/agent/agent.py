...
# Пересылка событий с этого сервера — тот же журнал, что читает
# eventWatcher.ts на сервере с хабом, только свой для этой машины.
# Позиция — по числу строк, не по времени (часы могут быть рассинхронизированы).
# Файла может не быть вообще — не ошибка, просто нечего пересылать пока.
