...
# кириллица (2 байта/символ) и брайль-спиннер ниже (_spin_wait) выравнивались
# бы криво. "export LC_ALL=X 2>/dev/null || ..." тут не спасает — export
# никогда не возвращает ошибку, даже для полностью несуществующей локали
# (проверено), так что запасной вариант ниже никогда бы не сработал.
# Проверяем по locale -a, что локаль реально установлена, прежде чем её
# выбирать — иначе просто оставляем как есть (C/POSIX), не подставляем
# гарантированно нерабочее имя.
...
restart_ssh() {
    ...
    # "enable --now" на уже запущенной службе — no-op, конфиг не
    # перечитывается. Останавливаем явно (и сокет, и службу под обоими
    # именами), затем запускаем заново — гарантированно не no-op.
...
if is_done "step1_1"; then
    ...
    # Раньше 3 пинга шли строго по очереди без единого признака, что
    # скрипт вообще что-то делает — до ~9с (3 сервера × 3 попытки по 1с)
    # молчания в терминале, если все три недоступны. Теперь параллельно
    # (быстрее) и под спиннером — как и любая другая операция дольше
    # мгновенной в этом скрипте.
    PING_TMP=$(mktemp -d)
    (
        for i in "${!DNS_LIST[@]}"; do
            entry="${DNS_LIST[$i]}"
            ip1="${entry#*:}"; ip1="${ip1%%:*}"
            measure_ping "$ip1" > "$PING_TMP/$i" &
        done
        wait
    ) &
    ping_pid=$!
    _spin_wait "$ping_pid" "Проверяю доступность DNS-серверов"
    wait "$ping_pid"

    PINGS=()
    for i in "${!DNS_LIST[@]}"; do
        PINGS+=("$(cat "$PING_TMP/$i" 2>/dev/null)")
    done
    rm -rf "$PING_TMP"
...
#!/bin/bash
# Автоочистка: пакеты, кэш apt, старые логи. Лог — в /var/lib/nexus404-base
# (тот же DK_DIR, что и у инсталлятора, гарантированно существует).
apt-get autoremove -y >> /var/lib/nexus404-base/cleanup.log 2>&1
apt-get autoclean -y >> /var/lib/nexus404-base/cleanup.log 2>&1
journalctl --vacuum-time=7d >> /var/lib/nexus404-base/cleanup.log 2>&1
...
/var/lib/nexus404-base/cleanup.log {
    size 10M
    rotate 5
    compress
    missingok
...
EOF
    echo "${GREEN}[✓]${NC} Логротация для cleanup.log настроена (по достижении"
...
#!/bin/bash
# Тот же каталог, что у инсталлятора (/var/lib/nexus404-base,
# гарантированно существует), та же переменная, что в $HEALTHLOG ниже.
LOG="/var/lib/nexus404-base/health.log"
